...
from .serving.minimax_h3_video import DEFAULT_MLX_SERVE_BINARY
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    h3_video = subparsers.add_parser(
        "minimax-h3-video",
        help="Run the native-MLX MiniMax H3 video backend.",
    )
    h3_sub = h3_video.add_subparsers(dest="h3_command")
    h3_serve = h3_sub.add_parser(
        "serve",
        help="Validate and exec the native mlx-serve MiniMax H3 runtime.",
    )
    h3_serve.add_argument("--binary", default=DEFAULT_MLX_SERVE_BINARY)
    h3_serve.add_argument("--model", required=True)
    h3_serve.add_argument("--host", default="127.0.0.1")
    h3_serve.add_argument("--port", type=int, default=11_241)
    h3_serve.add_argument("--rank", type=int, default=0)
    h3_serve.add_argument("--world-size", type=int, default=1)
    h3_serve.set_defaults(handler=_run_h3_video_serve)
...
def _run_h3_video_serve(args: argparse.Namespace) -> int:
    from .serving.minimax_h3_video import exec_native_h3
