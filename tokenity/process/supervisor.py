...
from tokenity.node_agent.recovery import process_start_identity
...
class _AdoptedProcess:
    """A PID handle with a start-time fence for a process from an older Agent."""

    stdin = None

    def __init__(self, pid: int, start_identity: str):
        self.pid = pid
        self.start_identity = start_identity

    def poll(self) -> int | None:
        if process_start_identity(self.pid) != self.start_identity:
            return 0
        try:
            completed = subprocess.run(
                ["/bin/ps", "-o", "stat=", "-p", str(self.pid)],
                check=True,
                capture_output=True,
                text=True,
                timeout=2,
            ...
            return 0
        return 0 if completed.stdout.strip().startswith("Z") else None

    def send_signal(self, sig: signal.Signals) -> None:
        if process_start_identity(self.pid) != self.start_identity:
            raise ProcessLookupError(
                f"PID {self.pid} no longer belongs to the adopted process."
            )
        os.kill(self.pid, sig)

    def send_group_signal(self, sig: signal.Signals) -> None:
        if process_start_identity(self.pid) != self.start_identity:
            raise ProcessLookupError(
                f"PID {self.pid} no longer belongs to the adopted process group."
            )
...
class RoleSupervisor:
    ...
    def adopt(
        ...
        *,
        ...
        """Take over a previously journaled runtime after fenced verification."""

        if process_start_identity(pid) != start_identity:
            raise ValueError("Process start identity changed before adoption.")
        key = (role, instance_id)
        with self._lock:
            existing = self._processes.get(key)
            if existing is not None and existing.poll() is None:
                if existing.pid == pid and self._operation_ids.get(key) == operation_id:
                    return self.status(role, instance_id=instance_id)
                raise RuntimeError(f"Role {role} for {instance_id} is already managed.")
            self._processes[key] = _AdoptedProcess(pid, start_identity)  # type: ignore[assignment]
            self._commands[key] = list(command)
            if log_path is not None:
                self._logs[key] = Path(log_path)
            self._new_sessions[key] = True
...
def _terminate_process_group(process: subprocess.Popen[bytes], sig: signal.Signals) -> None:
    if process.stdin is not None:
        try:
            ...
            pass
    try:
        if isinstance(process, _AdoptedProcess):
            process.send_group_signal(sig)
            return
...
def _related_process_markers(command: list[str] | None) -> list[str]:
    ...
    for match in re.finditer(
        r"tokenity\s+minimax-h3-video\s+serve\s+--binary\s+\S+\s+--model\s+('([^']+)'|\"([^\"]+)\"|(\S+))",
        command_text,
    ):
        model = next(group for group in match.groups()[1:] if group)
        markers.append(f"mlx-serve --model {model}")
    return markers
