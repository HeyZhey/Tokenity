...
def validate_sse_completion(
    *,
    done_received: bool,
    trailing_buffer: bytes,
    content_fragments: list[str],
) -> None:
    if trailing_buffer.strip():
        raise RuntimeError("Malformed SSE: stream ended with an incomplete line")
    if not done_received:
        raise RuntimeError("Malformed SSE: stream ended without [DONE]")
    if not "".join(content_fragments).strip():
        raise RuntimeError("Empty output: stream contained no answer text")
        ...
        with opener.open(request, timeout=timeout) as response:
            ...
            while True:
                ...
                while b"\n" in buffer:
                    ...
                    if data == "[DONE]":
                        if done_received:
                            raise RuntimeError("Malformed SSE: duplicate [DONE]")
                        done_received = True
                    ...
                    if not data:
                        continue
                    if done_received:
                        raise RuntimeError("Malformed SSE: data received after [DONE]")
                    ...
                    if isinstance(content, str) and content:
                        ...
                        content_event_times.append(received_at)
                        first_content_at = first_content_at or received_at
            validate_sse_completion(
                done_received=done_received,
                trailing_buffer=buffer,
