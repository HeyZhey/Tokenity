...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=parse_node,
        required=True,
        help="Repeat for each node: id,agent_url,lan_ip,rdma_device,rdma_ip",
    )
    parser.add_argument(
        "--model-path",
        default=os.environ.get("TOKENITY_PREFLIGHT_MODEL_PATH"),
        required="TOKENITY_PREFLIGHT_MODEL_PATH" not in os.environ,
    )
    parser.add_argument(
        "--python-path",
        default=os.environ.get("TOKENITY_RUNTIME_PYTHON"),
        required="TOKENITY_RUNTIME_PYTHON" not in os.environ,
    )
