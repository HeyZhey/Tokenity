...
<p align="center">
  A Mac-native control plane for stable MLX language and MiniMax H3 video inference across Apple-silicon machines.
</p>
...
> [!IMPORTANT]
> Tokenity is an early development project. This unified baseline combines the
> recoverable multi-instance language runtime with the native MiniMax H3 TP2
> video path. Large-model execution still depends on matching runtime,
> checkpoint, memory, and RDMA state on every selected Mac.
...
- Native MiniMax H3 single-Mac and two-Mac TP2 video generation, typed startup,
  SSE progress, playable artifact creation, fixed-seed benchmarking, and
  coordinated rank shutdown.
- Journal-backed Agent restart recovery for language instances and fenced,
  fail-closed cleanup for native H3 processes that cannot prove a Tokenity
  runtime epoch after an Agent restart.
...
flowchart LR
    ...
    UI -->|"isolated H3 control :9100/:9200"| H0["MiniMax H3 rank 0\nnative video API"]
    H0 <-->|"H3 TP2 · JACCL/RDMA :30096+"| H1["MiniMax H3 rank 1\nisolated Agent"]
...
- Optional isolated MiniMax H3 worker Agent reachable on TCP port `9200`.
...
1. Open **Cluster**. Tokenity automatically discovers Node Agents on local
   private IPv4 subnets and repairs saved endpoints whose DHCP address changed;
   **Discover Macs** triggers an immediate scan. If multicast or subnet scanning
   is restricted, enter a DNS name or Agent URL under **Connect by URL**.
2. Select any newly discovered Macs that will participate. Previously selected
   Macs remain selected when their address changes. Remote slots have no
   built-in IP address and become selected only after a verified Agent claims
   them.
3. Confirm both nodes report ready memory and RDMA/network state.
4. Use the recommended runtime: **Tokenity Distributed → Thunderbolt RDMA**.
5. Select **Create Cluster**.
6. Open **Models**, scan the shared model directory, and review its metadata.
7. Optionally open **Configure** to adjust runtime and generation settings.
8. Select **Load** and wait until the model state becomes **Loaded**.
9. Use **Chat**, or open **API Access** to connect another application.
...
Only **Quit Tokenity** exits the app. Quitting cancels active requests and stops
the UI-owned H3 runtime; Agent-owned language instances receive a restart grace
lease and remain recoverable by the next app process.
...
| `POST /v1/video/generations` | MiniMax H3 video generation, including SSE progress streams. |
...
```bash
TOKENITY_HOST="${TOKENITY_HOST:?set the coordinator DNS name or IP}"
...
tokenity minimax-h3-video serve --binary /path/to/mlx-serve \
  --model /path/to/MiniMax-H3-MLX-8bit --host 0.0.0.0 --port 11241
```

Use `tokenity <command> --help` for all arguments.

The MiniMax H3 integration provides a supervised single-Mac native server, a
stable Node Agent video gateway, and a protocol-v1 two-Mac TP2 launch path with
checkpoint/hash/revision gates and rollback. The target two-Mac path is hardware
validated, including its 2,800-collective gate, 124-frame audio/video request,
and coordinated shutdown. TokenityControl's **Video** page can validate the H3
Agents, start the TP2 runtime, stream generation progress, preview the result,
and save a playable MOV plus raw evidence. See
[MiniMax H3 video inference](docs/minimax-h3-video.md) for the UI workflow,
commands, API payloads, and exact TP2 boundary.
...
All install roots, Agent endpoints, H3 paths, and RDMA settings are deployment
configuration; see [Deployment configuration](docs/deployment-configuration.md).
...
- Automatic LAN node discovery is implemented for private/link-local IPv4
  subnets. It preserves machine identity and selection across address changes,
  deduplicates multiple Agent ports, and persists repaired endpoints. The
  topology editor still starts from the validated sample configuration as a
  recoverable seed rather than treating those addresses as authoritative.
