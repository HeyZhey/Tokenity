...
> 唯一有效工作区：`/path/to/Tokenity-Stable`
...
```text
/path/to/Tokenity-Stable/apps/TokenityControl/.build/arm64-apple-macosx/debug/TokenityControl-Stable.app
```
...
旧的 `/path/to/MLX-Distributed` 只可作为只读参考，不能从那里构建或启动 Tokenity UI。
...
| --- | --- | --- | --- | --- |
| Coordinator | Mango | `<node-a-lan-ip>:9100` | `<node-a-rdma-ip>` | `rdma_en4` |
| Worker | Kiwi | `<node-b-lan-ip>:9100` | `<node-b-rdma-ip>` | `rdma_en5` |

共享安装路径：

| 用途 | 路径 |
| --- | --- |
| Python runtime | `${TOKENITY_RUNTIME_PYTHON}` |
| 正式 Tokenity 代码 | `${TOKENITY_CODE_ROOT}` |
| 模型目录 | `${TOKENITY_MODEL_ROOT}` |
...
curl --noproxy '*' -sS http://<node-a-lan-ip>:9100/v1/models
```
