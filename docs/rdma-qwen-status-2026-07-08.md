...
- UI/control project: `/path/to/Tokenity`
- Active app bundle: `/path/to/Tokenity/apps/TokenityControl/.build/arm64-apple-macosx/debug/TokenityControl.app`
- Legacy UI path to avoid: `/path/to/MLX-Distributed/apps/TokenityControl`
- `/path/to/MLX-Distributed` is not currently a git repository.
- No references to the retired Mac B LAN address were found.

## Machines

- Mac A: `<node-a-user>@<node-a-lan-ip>`
- Mac B: `<node-b-user>@<node-b-lan-ip>`
...
- A to B BatchMode SSH over Thunderbolt passed:
  - `ssh <node-b-user>@<node-b-rdma-ip> hostname`
...
```json
[
  {"ssh":"127.0.0.1","ips":["<node-a-rdma-ip>"],"rdma":[null,"rdma_en4"]},
  {"ssh":"<node-b-user>@<node-b-rdma-ip>","ips":[],"rdma":["rdma_en5",null]}
]
```
...
```bash
ssh <node-a-user>@<node-a-lan-ip> 'curl --noproxy "*" -sS -X POST http://127.0.0.1:9100/v1/node/stop-role -H "Content-Type: application/json" -d "{\"role\":\"distributed-openai\",\"timeout\":10}"'
```
