...
- `${TOKENITY_MODEL_ROOT}/Qwen3.5-4B-Native-MTP-4bit`
...
|---|---|---|---|---|
| Mac A | `<node-a-lan-ip>` | `rdma_en4` active | `<node-a-rdma-ip>` | shared Python; MLX 0.31.2; mlx-lm 0.31.3 |
| Mac B | `<node-b-lan-ip>` | `rdma_en5` active | `<node-b-rdma-ip>` | shared Python; MLX 0.31.2; mlx-lm 0.31.3 |

The exact interpreter on both ranks was
`${TOKENITY_RUNTIME_PYTHON}`. JACCL tests began
