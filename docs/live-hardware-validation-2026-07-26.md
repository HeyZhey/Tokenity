...
| 项目 | Mac A | Mac B |
|---|---|---|
| Node Agent | `192.168.5.23:9100` | `192.168.5.75:9100` |
| 统一内存 | 512 GiB | 512 GiB |
| Thunderbolt IP | `192.168.0.1` | `192.168.0.2` |
...
均来自 `/Users/Shared/TokenityRuntime/current/.venv/bin/python`。
...
早期若干 GLM 试验实际子进程仍来自 `/Users/Shared/TokenityCode`，没有加载本轮
...
- 按 Mac A、Mac B 顺序升级。每端安装前均保存
  `/Users/Shared/TokenityCode/tokenity` 备份；安装器返回成功并写入
...
已经安装到 Mac A `192.168.5.23:9100` 和 Mac B `192.168.5.75:9100`。
