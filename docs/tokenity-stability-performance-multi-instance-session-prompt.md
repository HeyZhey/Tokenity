...
- oMLX：`/path/to/MLX-Distributed/omlx-main-0713`
- exo：`/path/to/MLX-Distributed/exo-main-0713`
...
- SSH 调试地址：`<node-a-user>@<node-a-lan-ip>`
- Node Agent：`http://<node-a-lan-ip>:9100`
- 普通网络接口：`en4`
- RDMA 设备：`rdma_en4`
- Thunderbolt/RDMA IP：`<node-a-rdma-ip>`

### Mac B

- SSH 调试地址：`<node-b-user>@<node-b-lan-ip>`
- Node Agent：`http://<node-b-lan-ip>:9100`
- 普通网络接口：`en5`
- RDMA 设备：`rdma_en5`
- Thunderbolt/RDMA IP：`<node-b-rdma-ip>`

Mac B 的唯一当前普通网络地址是 `<node-b-lan-ip>`。任何旧 Mac B LAN 地址都属于废弃配置：

- 不得恢复旧地址。
- 不得在默认节点、fallback、迁移代码、sample、测试 fixture、文档、脚本或 UI 中重新加入旧地址。
- 不得因为旧缓存或旧构建产物将 Mac B 回写到其他地址。
- 对 Mac B 身份的兼容逻辑只能收敛到 `<node-b-lan-ip>`；不要保留陈旧地址 alias。
- `<node-b-rdma-ip>` 是 Mac B 的 Thunderbolt/RDMA 数据面地址，不是废弃 LAN 地址。
...
```text
${TOKENITY_MODEL_ROOT}/Qwen3.5-122B-A10B-4bit
```
...
```bash
ssh <node-a-user>@<node-a-lan-ip> hostname
ssh <node-b-user>@<node-b-lan-ip> hostname
```

如果确认 Mac A 的 Node Agent 未运行，可在调试期间临时启动：

```bash
ssh <node-a-user>@<node-a-lan-ip> 'mkdir -p "$HOME/Library/Application Support/Tokenity/logs"; nohup env PATH=${TOKENITY_RUNTIME_ROOT}/current/.venv/bin:/usr/bin:/bin:/usr/sbin:/sbin PYTHONPATH=${TOKENITY_CODE_ROOT} ${TOKENITY_RUNTIME_PYTHON} -u -m tokenity node-agent --host 0.0.0.0 --port 9100 > "$HOME/Library/Application Support/Tokenity/logs/node-agent.log" 2>&1 &'
```

如果确认 Mac B 的 Node Agent 未运行，可在调试期间临时启动：

```bash
ssh <node-b-user>@<node-b-lan-ip> 'mkdir -p "$HOME/Library/Application Support/Tokenity/logs"; nohup env PATH=${TOKENITY_RUNTIME_ROOT}/current/.venv/bin:/usr/bin:/bin:/usr/sbin:/sbin PYTHONPATH=${TOKENITY_CODE_ROOT} ${TOKENITY_RUNTIME_PYTHON} -u -m tokenity node-agent --host 0.0.0.0 --port 9100 > "$HOME/Library/Application Support/Tokenity/logs/node-agent.log" 2>&1 &'
```
...
- 所有工作、构建和启动来自 `/path/to/Tokenity-Stable`。
- 从未启动旧 `MLX-Distributed/apps/TokenityControl` UI。
- Mac B 只使用 `<node-b-lan-ip>`（LAN）和 `<node-b-rdma-ip>`（RDMA），没有恢复旧地址。
