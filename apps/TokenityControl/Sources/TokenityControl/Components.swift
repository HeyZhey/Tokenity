...
struct StatusPill: View {
    ...
    var body: some View {
        ...
        .overlay {
            Capsule()
                .stroke(color.opacity(0.38), lineWidth: 0.6)
        }
        .accessibilityLabel(text)
    }

    private var textColor: Color {
        tone == .neutral ? theme.secondaryText : color
    }
...
struct TokenityIconButtonStyle: ButtonStyle {
    @Environment(\.tokenityTheme) private var theme
    @Environment(\.isEnabled) private var isEnabled

    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            .foregroundStyle(isEnabled ? theme.secondaryText : theme.tertiaryText)
            .background(
                configuration.isPressed ? theme.hover : theme.surface,
