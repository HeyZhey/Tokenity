...
final class ResidentModelRecoveryTests: XCTestCase {
    func testLateBackgroundResidentSnapshotCannotOverwriteForegroundStop() async throws {
        let modelID = "Qwen3.5-122B-A10B-4bit"
        let instance = Self.instanceJSON(
            id: "instance-late-refresh",
            model: modelID,
            state: "ready",
            port: 18_000
        )
        var delaysRoutes = false
        var stopped = false
        var delayedRequest: URLRequest?
        var routesContinuation: CheckedContinuation<(Data, HTTPURLResponse), Never>?
                ...
                if path == "/v1/gateway/routes" {
                    if delaysRoutes {
                        ...
                        return await withCheckedContinuation { continuation in
                            routesContinuation = continuation
                        }
                    }
                    return Self.response(
                        for: request,
                        payload: Self.routesPayload(instances: stopped ? [] : [instance])
                    )
                }
                if path.hasSuffix("/quorum") {
                    return Self.response(
                        for: request,
                        payload: Self.quorumPayload(instanceID: "instance-late-refresh", ready: true)
                    )
                }
                if path == "/v1/node/instances/instance-late-refresh/stop" {
                    stopped = true
        ...
        let row = try XCTUnwrap(store.modelLibraryRows.first { $0.id == modelID })
        XCTAssertEqual(store.modelLoadStates[modelID], .loaded)

        delaysRoutes = true
        let refresh = Task { await store.refreshSelectedNodeStatus(showsActivity: false) }
        for _ in 0..<100 where routesContinuation == nil {
            await Task.yield()
        }
        XCTAssertNotNil(routesContinuation)

        await store.stopModel(row)
        let request = try XCTUnwrap(delayedRequest)
        routesContinuation?.resume(
            returning: Self.response(
                for: request,
                payload: Self.routesPayload(instances: [instance])
            )
        )
        await refresh.value
        ...
        XCTAssertEqual(store.modelLoadStates[modelID], .notLoaded)
    ...
    func testRefreshRecoversReadyReplicasAndChatUsesStableGateway() async throws {
            ...
            dataTransport: { request in
                ...
                if path == "/v1/node/heartbeat",
                   ...
                   let instanceID = object["instance_id"] as? String {
                    ...
                    if let ttl = object["ttl_seconds"] as? Double {
        ...
        XCTAssertEqual(chatURL?.host, "127.0.0.1")
        ...
        XCTAssertTrue(heartbeatTTLs.contains(120))
            ...
            dataTransport: { request in
                ...
                if path == "/v1/node/start-distributed-openai" {
                    ...
                    return Self.response(
                        for: request,
                        payload: """
                        {"instance_id":"\(instanceID)","operation_id":"operation-slow","api_base_url":"http://127.0.0.1:18000/v1"}
                        """
                    )
                }
                if path == "/v1/node/heartbeat" {
            ...
            dataTransport: { request in
                ...
                if path == "/v1/node/start-distributed-openai" {
                    ...
                    return Self.response(
                        for: request,
                        payload: """
                        {"instance_id":"\(loadingInstanceID)","operation_id":"operation-second","api_base_url":"http://127.0.0.1:18001/v1"}
                        """
                    )
                }
    ...
    func testSecondResidentLoadIsBlockedWhenAnySelectedAgentLacksCapability() async throws {
        ...
        let store = TokenityStore(dataTransport: { request in
            ...
            if path == "/v1/node/start-distributed-openai" {
                ...
                return Self.response(
                    for: request,
                    payload: "{\"instance_id\":\"\(instanceID)\",\"operation_id\":\"operation\",\"api_base_url\":\"http://127.0.0.1:\(port)/v1\"}"
                )
            }
            if path.hasPrefix("/v1/node/instances/"), path.hasSuffix("/quorum") {
                return Self.response(
    ...
    private static func instanceJSON(id: String, model: String, state: String, port: Int) -> String {
        """
        {"instance_id":"\(id)","operation_id":"operation-\(id)","requested_model_id":"\(model)","resolved_path":"/fixtures/tokenity/models/\(model)","execution_mode":"single","selected_nodes":["mac-a"],"world_size":1,"connection_mode":"ring","coordinator":"mac-a","http_port":\(port),"state":"\(state)","active_request_count":0}
        """
    }
