...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| 单机 · 512 GiB | 被安全内存准入阻止 | — | — | — | — | — |
| 双机 TP2 · 2 × 512 GiB | 2/2 ranks Ready | **23.28 tok/s** | **23.75 tok/s** | **1.63 s** | **2.06 s** | **5/5** |

单机任务在模型启动前被拒绝：保守估算需要 523.58 GB，而该节点可准入内存只有
394.59 GB。Tokenity 没有绕过安全检查，因此这里不会给出误导性的单机吞吐数据。
双机 TP2 测试中，每个 rank 观测到的模型内存峰值约为 200 GB；5 次请求完成后两个
rank 均保持健康，并在测试结束后干净停止。测试模型为 `GLM-5.2-mxfp4`，使用仓库
内的[流式 Benchmark 工具](scripts/benchmark-openai-stream.py)。

### MiniMax H3 视频生成

2026-08-26 的配对测试固定使用 `512×256`、124 帧、28 个采样步、seed 42 和
`fast=false`。每种拓扑分别通过 Agent 网关完成 1 次 cold 与 3 次 warm 请求，
均返回完整 RGB8 帧与立体声 PCM 音频：

| 指标 | 单机 | 双机 TP2 | 提升 |
| --- | ---: | ---: | ---: |
| Cold DiT 采样 | 316.203 s | 181.385 s | **1.743×** |
| Cold 端到端 | 347.126 s | 201.497 s | **1.723×** |
| Warm DiT 均值 · n=3 | 315.024 s | 182.076 s | **1.730× · 耗时减少 42.2%** |
| Warm 端到端中位数 · n=3 | 338.744 s | 204.544 s | **1.656× · 耗时减少 39.6%** |
| Warm 视频解码均值 | 9.937 s | 10.002 s | 仅 Rank 0 |
| Warm 音频解码均值 | 0.570 s | 0.571 s | 仅 Rank 0 |

8 次请求均完成 31 个进度事件、124 帧视频与有效音频；每种拓扑内部的 4 次输出
均保持逐字节确定性。单机全权重与 TP2 分片使用不同数值路径，因此跨拓扑并非
bit-exact；完整 124 帧 RGB 输出的对比为 SSIM `0.910`、PSNR `27.26 dB`。由于
单机有一次网关输出传输离群值，端到端采用 warm 中位数；后端 DiT 数据始终稳定。
两个 TP2 rank 最终均以 code 0 退出，两台 Agent 的端口和内存预留均已释放。
