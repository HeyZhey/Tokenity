...
固定请求使用 `512×256`、124 帧、28 个采样步、seed 42 和 `fast=false`，通过
Agent 网关完成并返回完整 RGB8 帧与立体声 PCM 音频：

| 指标 | 单机 | 双机 TP2 | 提升 |
| --- | ---: | ---: | ---: |
| DiT 采样 | 301.891 s | 187.384 s | **1.611×** |
| 端到端 | 322.583 s | 204.512 s | **1.577×** |
| 视频解码 | 10.036 s | 9.976 s | 仅 Rank 0 |
| 音频解码 | 0.910 s | 0.835 s | 仅 Rank 0 |
